...
- 카드는 한 장씩 놓입니다: 블랙잭은 1.25초, 홀덤(홀 카드·번 카드·보드)은 1.7초 간격입니다. 홀덤은 카드가 연달아 나가는 일이 많아(번 카드와 플롭 세 장), 블랙잭 히트처럼 카드마다 손을 제 속도로 슈에 되돌린 뒤 다음 카드를 집습니다. 카드를 놓고 손을 되돌리는 중에는 뒤집기(플롭 등)가 동작을 끊지 않습니다. 딜러 영상은 카드마다 슈에서 카드를 꺼내 테이블에 내려놓고 손을 다시 슈 위로 가져가는 동작을 이어서 하고, 카드가 슈를 떠나는 순간과 좌석에 놓이는 순간이 영상의 꺼내기·내려놓기와 맞습니다. 각 카드는 슈에서 420ms 동안 날아오고, 히트 카드는 누른 뒤 950ms에 도착합니다. 홀덤은 스트리트마다 번 카드를 먼저 놓고, 플롭은 뒷면으로 놓은 뒤 함께 뒤집습니다. 딜러 홀 카드와 쇼다운 카드는 정해진 순서에 맞춰 뒤집습니다. 홀덤이 폴드로 끝나 카드가 가려진 채 끝나면, 다음 핸드를 시작하기 전까지 **내 카드 보여주기**로 내 홀 카드를 모두에게 공개할 수 있습니다(이긴 사람·폴드한 사람 모두). 카드가 모두 놓인 뒤에야 결과·칩·기록이 나타납니다. 1번 좌석은 슈 옆 첫 번째 베이스에 앉고, 실제 테이블처럼 오른쪽에서 왼쪽으로 딜합니다.
...
- **관리 로그 채널**(`/마피아로그채널`)에는 관리자 작업 말고도 코인이 오가는 일이 5초마다 모여 종류별로 올라갑니다: 주식 로그(체결·지정가 주문·취소·만료, 청약·배정, 회사 설립·상장·배당·유상증자·자사주·위험도·해산, 배당·청산 지급, 분기 실적, 관리종목·파산·상장폐지), 카지노 로그(바이인·캐시아웃), 코인 로그(출석·구조금·참여 보상·미션·업적).

## 코인 벌기

코인을 버는 방법은 출석(`/출석`, 하루 10,000), 마피아 배팅, 스타플레이어 상금, 카지노·주식에 더해 아래 네 가지입니다. 이 보상은 새로 발행하는 코인이라, 부계정으로 찍어 내지 못하게 하루 한도와 한 번뿐인 업적으로 총량을 묶었습니다. 금액은 모두 웹 설정(`/마피아웹설정`)에서 바꾸고 0이면 끕니다. 발행한 누적은 `/금고`에 나옵니다.

| 방법 | 기본값 |
|---|---|
| **마피아 참여 보상**: 판을 끝까지 하면 받고, 이긴 쪽은 더 받습니다. 판 결과 뒤에 안내가 뜹니다 | 1,000 (+승리 1,000), 하루 5판까지 |
| **일일 미션** (`/미션`): 마피아·카지노·주식에서 하나씩, 날마다 바뀌는 미션 세 개. 끝낸 것은 `/미션`을 쓰면 바로 받습니다 | 하나 2,000, 세 개 모두 +3,000 |
| **업적** (`/업적`): 트랙 20개·단계 70여 개를 오래 이어 갑니다 — 마피아 판 수(1~1,000판), 승리(1~500승), 최고 연승, 스타플레이어, 해 본 직업, 팀별 승리(시민·마피아·교주·조커), 플레이 시간, 최고 레이팅, 카지노 판 수·잭팟, 주식 체결 수·실현 수익, 회사 설립·상장, 출석 일수·최장 연속 출석, 미션 올클리어 일수. 단계마다 한 번씩 받고, 개편 전에 받은 업적은 오른 만큼 차액을 한 번 더 줍니다 | 단계당 5,000~800,000 (배율 100%) |
| **연속 출석 보너스**: `/출석`을 하루도 빼지 않으면 | 7일마다 +10,000, 30일마다 +30,000 |
...
- **플레이어 회사**: `/회사 설립`(자본금 100만 이상)으로 세우고, 1게임일 뒤 `/회사 상장`으로 공모 청약을 열어 상장합니다. 공모가가 주당 순자산에 가까우면 기관이 공모 주식의 최대 30%를 받아 가고, 이 물량이 상장 뒤 시장에서 거래됩니다. 배당·유상증자·자사주·위험도로 경영하고, 자본잠식이 이어지면 관리종목을 거쳐 상장폐지, 자본이 바닥나면 파산합니다. 자사주 매입은 하루 동안 장내에서 고르게 사서 소각하며, 그동안 회사가 현재가에 매수 호가를 내 주주가 바로 팔 수 있습니다. 종목 뉴스는 플레이어 회사에 더 자주(종목당 실제 하루 약 3건) 나옵니다.
...
- **증권 사이트(마피아증권)**: 카지노와 따로 떨어진 밝은 화면의 웹사이트입니다. `/주식 증권`이 주는 개인 링크(`/stocks/<토큰>`, 12시간 유효)에서 Discord 주식 명령으로 하는 일을 모두 합니다: 차트·호가창·주문·잔고, 공모주 청약·청약 취소, 유상증자 신주인수, 회사 경영, 주식 재산 순위. 관리자가 받은 링크에는 '관리' 탭이 더 있어 거래정지·재개, 시장 집계, 게임일 넘기기, 시세판·뉴스 채널 연결(채널 ID)을 사이트에서 합니다. 프론트엔드는 `stocks-web/`(Vite + React)이고 Activity 서버와 같은 포트에서 서비스되며, `build.rs`가 `stocks-web/dist`를 바이너리에 내장합니다 (`MAFIA_SKIP_STOCKS_BUILD`로 생략, `STOCKS_STATIC_DIR`로 디스크의 dist 사용). API는 `GET /stocks/api/{state,candles,ranking}`, `POST /stocks/api/{order,cancel,subscribe,unsubscribe,company,admin}` (개인 링크 토큰을 Bearer로, `admin`은 관리자 링크만).
- **채널**: `/주식패널`을 쓴 채널(또는 사이트 관리 탭에서 정한 채널)에 1분마다 고쳐지는 시세판이, `/주식뉴스채널`로 정한 채널에 모든 뉴스·공시가 5초 안에 올라옵니다: 회사 설립, 공모 청약 시작·상장·무산, 실적, 배당·유상증자·자사주 결정과 완료, 위험도 변경, 관리종목·파산·해산·상장폐지, 관리자 거래정지·재개, 서킷브레이커, 게임일 넘기기, 경제·종목 뉴스와 루머 (종목별 변동성 완화장치 정지는 잦아서 뺍니다).
...
| `/출석` | 하루 한 번 코인 받기, 연속 출석 7일·30일마다 보너스 | 누구나 |
| `/미션` | 오늘의 미션 세 개(마피아·카지노·주식) 진행 보기, 끝낸 미션 보상 받기 | 누구나 |
| `/업적` | 업적 목록과 진행 보기, 새로 이룬 업적 보상 받기 | 누구나 |
...
| `/주식 증권` | 증권 사이트(마피아증권) 개인 링크 (관리자가 받으면 사이트에 관리 탭이 생김) | 누구나 |
| `/회사 설립·정보·상장·배당·유상증자·신주인수·자사주·위험도·소개·해산` | 플레이어 회사 세우기·상장·경영 | 누구나 (경영은 대표) |
| `/주식관리 [거래정지/거래재개/시장 집계 보기/게임일 넘기기] [종목] [일수]` | 종목 거래정지·재개, 시장이 만든·없앤 코인 집계, 게임일 넘기기(1~24일, 시장 시계를 다음 게임일로 옮기고 그 사이 시세·청약·배당·보호예수를 바로 처리) | 관리자 |
...
`/마피아웹설정` 명령어로 관리자 전용 설정 편집 페이지의 링크를 발급받을 수 있습니다 (10분 유효). 설정은 기본, 게임 진행, 직업 구성, 특수 직업, 코인 보상, 카지노·잭팟, 금고·환급·구조금, 주식, 내신 쿠폰 카테고리로 나뉘어 탭마다 따로 저장하며, 저장하면 그 카테고리의 칸만 바뀝니다 (저장할 때 직업 구성처럼 다른 카테고리와 얽힌 조건도 함께 확인합니다). 링크는 10분 동안 여러 번 쓸 수 있고, 다 바꾸면 **설정 마치기**로 바로 닫습니다.
