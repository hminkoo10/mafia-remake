...
- 카드는 한 장씩 600ms 간격으로 놓입니다(블랙잭, 홀덤 홀 카드는 520ms). 각 카드는 슈에서 420ms 동안 날아오고, 히트 카드는 650ms 뒤 도착합니다. 홀덤은 스트리트마다 번 카드를 먼저 놓고, 플롭은 뒷면으로 놓은 뒤 함께 뒤집습니다. 딜러 홀 카드와 쇼다운 카드는 정해진 순서에 맞춰 뒤집습니다. 카드가 모두 놓인 뒤에야 결과·칩·기록이 나타납니다. 1번 좌석은 슈 옆 첫 번째 베이스에 앉고, 실제 테이블처럼 오른쪽에서 왼쪽으로 딜합니다.
