...
- 카드는 한 장씩 1.25초 간격으로 놓입니다(블랙잭·홀덤 모두, 실제 딜러 속도). 딜러 영상은 카드마다 슈에서 카드를 꺼내 테이블에 내려놓고 손을 다시 슈 위로 가져가는 동작을 이어서 하고, 카드가 슈를 떠나는 순간과 좌석에 놓이는 순간이 영상의 꺼내기·내려놓기와 맞습니다. 각 카드는 슈에서 420ms 동안 날아오고, 히트 카드는 누른 뒤 950ms에 도착합니다. 홀덤은 스트리트마다 번 카드를 먼저 놓고, 플롭은 뒷면으로 놓은 뒤 함께 뒤집습니다. 딜러 홀 카드와 쇼다운 카드는 정해진 순서에 맞춰 뒤집습니다. 카드가 모두 놓인 뒤에야 결과·칩·기록이 나타납니다. 1번 좌석은 슈 옆 첫 번째 베이스에 앉고, 실제 테이블처럼 오른쪽에서 왼쪽으로 딜합니다.
