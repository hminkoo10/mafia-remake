...
3. 웹에서 좌석(6석)을 고르고 코인으로 **바이인 5,000~20,000**(100 단위)을 하면 테이블 칩이 됩니다. 한 번에 한 테이블에만 앉을 수 있습니다
...
| 명령어 | 설명 | 권한 |
|--------|------|------|
| `/카지노테이블생성 [종류] [이름]` | 홀덤/블랙잭 테이블 생성 + 전용 채널 (이름 2~24자, 최대 12개) | 관리자 |
| `/카지노테이블닫기 [테이블]` | 테이블을 닫고 칩을 코인으로 돌려준 뒤 채널 삭제 | 관리자 |
| `/카지노테이블목록` | 열려 있는 테이블과 인원·단계 | 누구나 |
| `/카지노입장 [테이블]` | 개인 링크 발급 (비우면 첫 테이블, 본인에게만 표시) | 누구나 |
| `/카지노상태` | 내 코인, 앉은 테이블, 하우스 누적 | 누구나 |

### Discord 채널 중계

- 고정 상태 임베드: 단계, 팟/딜러 카드, 좌석별 칩·베팅, 딜러 안내. 변경이 있으면 테이블당 1.5초 간격으로 갱신
...
- 2~6인 노 리밋 텍사스 홀덤, 블라인드 **50/100**, 헤즈업은 버튼이 스몰 블라인드
- 액션 제한 **30초**. 시간 초과 시 체크 가능하면 체크, 아니면 폴드. 2회 연속 초과하면 자리 비움(웹에서 복귀)
- 사이드팟 정산, 나눠지지 않는 칩은 버튼 왼쪽부터

### 블랙잭 규칙 (에볼루션 스타일)

- 8덱, 딜러는 소프트 17 스탠드(S17), 블랙잭 3:2, 딜러 피크(에이스면 인슈어런스 후 확인)
- 베팅창 **15초** (100~5,000, 100 단위). 베팅 가능한 참가자가 모두 걸면 바로 딜. 액션 30초, 초과 시 스탠드
- 더블은 처음 두 장에서(스플릿 후에도 가능), 스플릿은 1회, 에이스 스플릿은 한 장씩만, 서렌더 없음
- 인슈어런스: 딜러 에이스일 때 **10초** 안에 결정, 베팅의 절반, 딜러 블랙잭이면 2:1
- 사이드베팅(100~2,500, 100 단위)은 딜 직후 정산
...
- 카드는 시간차로 열리고 그동안 액션이 막힙니다 (`src/casino/table.rs` 상수, 테스트에서는 0): 카드당 380ms, 스트리트 전 900ms, 쇼다운 좌석당 800ms, 딜러 드로우 900ms, 정산 전 700ms
