...
/// 제출 즉시 전달할 비밀 알림(사탐 추적, 공갈 통보 등)을 바로 DM으로 보낸다
/// (밤 행동 제출 직후 호출).
pub(crate) async fn deliver_live_notices(
    ctx: &serenity::Context,
    running: &Arc<RwLock<RunningGame>>,
) {
    let notices = running.write().await.game.take_live_notices();
...
pub async fn handle_contractor_submit(
    ...
    deliver_live_notices(ctx, &running).await;
    if done {
