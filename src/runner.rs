...
pub fn role_short_guide(role: Role) -> &'static str {
    match role {
        ...
        Role::Police => {
            "밤마다 한 명을 조사해 즉시 결과를 확인합니다. 결과는 제출 때 한 번만 오고, 대상은 제출 후 바꿀 수 없습니다."
        }
        ...
        Role::Detective => {
            "실시간 추적: 대상을 고른 순간부터 그 사람의 손이 누구를 향하는지 보이고, 손을 바꾸면 즉시 알림이 옵니다. 마피아를 추적하면 마피아팀이 과반으로 정한 처형 대상이 보입니다. 대상은 밤마다 한 번만 고를 수 있고 제출 후에는 바꿀 수 없습니다."
        }
        Role::CivilServant => {
        ...
        Role::Soldier => {
            "마피아 공격을 한 번 버티고, 불침번으로 스파이의 첩보·도둑의 도벽·사기꾼의 사기·청부업자의 청부를 막아내며 그 정체를 알아냅니다. 저주(개구리) 상태에서는 둘 다 멈춥니다."
        }
        Role::Spy => {
            "밤마다 한 명의 직업을 알아내고(결과는 제출 즉시 한 번만), 마피아를 찾아내면 그 밤 한 번 더 첩보를 사용합니다."
        }
        Role::Fraudster => {
        ...
        Role::Witch => {
            "밤에 대상을 개구리로 저주해 다음 밤까지 직업 능력(방탄·2표 같은 상시 능력 포함)과 채팅을 막습니다."
        }
        Role::Scientist => {
