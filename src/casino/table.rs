...
pub mod live_timing {
    ...
    /// 홀덤 카드(홀 카드·번 카드·보드)의 착지 간격. 블랙잭 히트처럼 카드마다 꺼내 내려놓고(약 0.81초)
    /// 손을 제 속도(약 1.2배속, 0.89초)로 슈에 되돌린 뒤 다음 카드를 집는다. 블랙잭 첫 딜 간격(1.25초)으로
    /// 이어 나누면 손을 2.5배속으로 급히 되돌려, 번 카드와 플롭처럼 연달아 나갈 때 손이 끊겨 보였다.
    pub const HOLE_CARD_MS: i64 = 1_700;
