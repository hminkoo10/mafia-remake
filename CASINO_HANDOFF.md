...
## 결과 박스·전체화면 독·딜러 동작 전체 (2026-09-24 밤 늦게, Claude + Codex)

- **결과 박스** (`73909ef`): 가운데 정렬(transform)을 등장 애니메이션이 덮어 0.3초 동안 오른쪽 아래에 기울어져 떴다가 제자리로 튀었다(133px). `translate` 속성으로 가운데 정렬, 등장은 opacity·scale만.
- **전체화면 독** (`0f6bef9`, Codex): 홀덤 내 차례에 폴드·콜·레이즈가 190px 독에 잘려 누를 수 없었다. 독을 고정 높이(데스크톱 210px, 휴대폰 280px)로 하고 모든 단계를 그 안에 배치. 단계가 바뀌어도 테이블 높이가 같다(1366×768 490px, 1440×900 622px, 390×844 500px).
- **딜러 동작 전체** (`d93774e` 외): 딜 영상을 카드를 테이블에 내려놓는 데까지(원본 0~30프레임) 늘리고, 카드가 슈를 떠날 때 12프레임·좌석에 놓일 때 28프레임이 오게 약 1.59배속으로 튼다. 되돌리기 영상(`sophia-return`, 로컬 생성)을 새로 만들어 카드마다 손을 슈 위로 되돌린다(다음 카드 전에 끝나게 1~2.5배속, 마지막 카드 뒤 1.25배속 → 대기). 카드 간격 1.25초(블랙잭·홀덤). 실측(GPU 한가할 때): 출발 순간 0.49~0.50초, 착지 순간 1.21~1.23초(감지 지연 40ms 포함), 층 흐름 대기→딜→되돌리기→…→대기.
- 되돌리기 첫 생성(seed 473)은 손이 하얗게 뭉개져 빈손 프롬프트·seed 911로 다시 만들었다.
- 주의: 로컬 영상 생성(내장 GPU)을 돌리는 동안에는 Chrome 영상 디코딩이 늦어져 싱크 측정이 흔들린다(출발 순간 0.02~0.49초). 측정은 생성이 끝난 뒤에 한다.
- Adobe Firefly(사용자 계정)로 첫·마지막 프레임을 고정한 영상을 만들려 했으나, 교육용 프로필(관리자 설정)이라 영상 생성 권한이 없어 중단했다(크레딧 사용 없음).

## 전체화면·영상 싱크·비공개 채널·느린 요청 로그 (2026-09-24 밤, Claude + Codex)

- **영상 싱크** (`618bf3a`): 딜 영상을 카드처럼 서버 시각으로 튼다(`schedule.ts` `dealClipAt`). 카드마다 영상 12프레임(0.50초, 카드가 슈에서 빠지는 순간)이 카드 출발 시각에 오도록 시작하고, 연달아 나가는 카드는 1~1.8배속으로 맞춘다. 첫 카드도 맞도록 `DEAL_LEAD_MS`를 650→950ms로 늘렸다(히트 카드는 누른 뒤 950ms에 도착). 실측: 모든 카드 출발 순간 영상 위치 0.48~0.55초.
- **전체화면** (`baf117b`): 베팅하려고 스크롤을 오가지 않게 전체화면에서 테이블과 베팅 독이 한 화면에 들어간다. 제목·푸터를 숨기고 44px 막대(코인·아이콘), 독은 한 줄(휴대폰 두 줄). 베팅 단계 실측: 1366×768 테이블 525 + 독 175, 390×844 537 + 239, 문서 스크롤 없음.
- **"서버 응답이 늦어요" 오탐** (`048313e`): 백그라운드 새로 받기는 한 번 재시도하고, 10초 안에 상태·하트비트가 왔으면 띄우지 않는다. 숨긴 동안 시작된 요청은 무시.
- **비공개 테이블 채널** (`9ca12c4`): `@everyone` 보기 거부, 봇 허용, 착석자만 보기·채팅. 릴레이가 좌석 변화에 맞춰 멤버 권한을 맞추고, 시작할 때 기존 채널도 맞춘다. 웹훅 채팅 양방향은 그대로. **배포 후 실제 Discord에서 확인 필요**(개발 서버에는 Discord가 없다).
- **느린 요청 로그** (`2f90f70`): 명령·상태 요청이 1초 이상이면 `casino slow request:` 한 줄(테이블 잠금·코인 잠금 대기, stats/casino 저장, 응답 생성 시간). `tick_all`·웹소켓 상태 생성은 500ms 이상. 코드 읽기로는 몇 초씩 멈출 곳을 찾지 못했다 → 배포 후 이 로그로 원인을 본다. Codex가 코인 이동 저장을 백그라운드로 바꿨으나 저장 순서가 뒤집혀 코인 스냅샷이 되돌아갈 수 있어 원래대로(기다려 저장) 되돌렸다.
- 검토에서 고친 Codex 결과물: 딜 클립을 객체로 구독해 React가 끝없이 다시 그리던 문제(문자열 키로 구독, `useServerValue`는 원시값만), DashMap 가드를 await 너머로 쥐던 채널 동기화, 테스트 모듈 import 누락.
- 사고: OpenClaw 임시 폴더를 정리하다 실행 중인 게이트웨이의 플러그인 폴더를 지워 Codex 호출이 실패했다 → `openclaw gateway restart`로 복구. 자동 청소(작업 스케줄러 "OpenClaw Temp Cleanup", `~/.openclaw/clean-tmp.ps1`)는 실행 중인 OpenClaw node 프로세스보다 먼저 만든 폴더만 지운다.
- 검증: Rust 테스트 271 + 158 + 3 + 3, 새 clippy 경고 없음. 웹 tsc, node 테스트 37개, build 통과.
